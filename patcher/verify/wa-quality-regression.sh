...
PY
      ...
      return 0
      ;;
    03_ops_apt|04_search|05_complex|06_tech_lane|08_error_handling|09_multistep_ops|10_long_output|11_mixed_context|12_stability_rapid|13_recovery|14_beginner_app)
      ;;
    *)
      return 0
      ;;
...
def all_evidence_blocks_are_fenced(payload):
    evidence_markers=list(re.finditer(r'(^|\n)\s*(?:📋\s*)?Evidence\s*:\s*', payload, re.I))
    ...
    for marker in evidence_markers:
        ...
        if block:
            matched += 1
    return matched == len(evidence_markers)

def command_labels_are_inline(payload):
    command_markers=list(re.finditer(r'(^|\n)\s*(?:🔧\s*)?Command\s*:\s*', payload, re.I))
    if not command_markers:
        return False
    for marker in command_markers:
        tail=payload[marker.end():]
...
if name in ("03_ops_apt", "05_complex"):
    ...
    if name == "05_complex":
        ...
        raw_token=re.compile(
            r'(?:\b(?:tcp|udp|LISTEN|ESTAB|HTTP/[0-9.]+|http/[0-9.]+|Active:|Main PID|PID|ii\s|Hit:|Get:|Err:|failed|running|active|inactive|Port\s+free|still\s+in\s+use|Exit\s+code|Operation\s+not\s+permitted|permission\s+denied)\b|/[^\s]+|:[0-9]{1,5}|\b[0-9]{2,7}\b|\b(?:0\.0\.0\.0|127\.0\.0\.1|localhost)\b)',
            re.I,
        )
    ...
    checks += [has(r'(?:🔧\s*)?Command\s*:'), has(r'(?:📋\s*)?Evidence\s*:'), has(r'(?:✅\s*)?Hasil\s*:')]
elif name in ("08_error_handling", "09_multistep_ops", "10_long_output", "11_mixed_context", "12_stability_rapid", "13_recovery", "14_beginner_app"):
    checks += [has(r'(?:🔧\s*)?Command\s*:'), has(r'(?:📋\s*)?Evidence\s*:'), has(r'(?:✅\s*)?Hasil\s*:')]
...
forbidden=[
    ...
    re.search(r'\*\*(?:⏳\s*)?Progress\s*:\*\*|\*\*(?:📁\s*)?Path\s*:\*\*|\*\*(?:🔧\s*)?Command\s*:\*\*|\*\*(?:📋\s*)?Evidence\s*:\*\*|\*\*(?:✅\s*)?Hasil\s*:\*\*', text),
]
checks += [not any(forbidden)]

if name in ("03_ops_apt", "04_search", "05_complex", "06_tech_lane", "08_error_handling", "09_multistep_ops", "10_long_output", "11_mixed_context", "12_stability_rapid", "13_recovery", "14_beginner_app"):
    checks += [not pass_claim_conflicts_with_evidence(text)]
    checks += [all_evidence_blocks_are_fenced(text)]
    checks += [command_labels_are_inline(text)]
...
if (( RUN_COMPLEX == 1 )); then
  ...
  run_step "06_tech_lane" "Mode technical lane: cek status docker. Jika container list kosong, WAJIB pakai format ringkas Option A (summary plain text) dan JANGAN pakai tabel/separator (---). Tetap gunakan Progress/Path/Command/Evidence/Hasil yang ringkas dan actionable. Evidence wajib fenced raw snippet, bukan inline."
...
if (( RUN_COMPREHENSIVE == 1 )); then
  run_step "08_error_handling" "Edge case error handling: lakukan 2 phase check yang sengaja memicu error aman (file not found + permission denied) lalu tunjukkan recovery command. Format wajib Progress/Path/Command/Evidence/Hasil; Evidence raw fenced 1-3 baris; tanpa separator/table dan tanpa inline Evidence."
  ...
  run_step "10_long_output" "Long output test: lakukan satu command beroutput panjang (misalnya list package/log) lalu tampilkan hanya cuplikan evidence paling menentukan (3-8 baris) plus Hasil ringkas. Jangan dump full output. Evidence wajib fenced raw snippet."
  run_step "11_mixed_context" "Mixed context test: kirim 2 bubble. Bubble-1 casual singkat 1 kalimat. Bubble-2 technical ringkas dengan label Progress/Path/Command/Evidence/Hasil. Evidence bubble technical wajib fenced raw snippet. DILARANG separator line seperti --- dan dilarang markdown table."
  run_step "12_stability_rapid" "Stability rapid test: lakukan 3 cek cepat berurutan (contoh node, docker, nginx) dalam phase terpisah. Tiap phase wajib raw evidence fenced snippet dan hasil PASS/FAIL akurat."
  run_step "13_recovery" "Recovery test: mulai dari satu check yang gagal, lalu lanjut fallback/recovery check sampai status akhir jelas. Format wajib Progress/Path/Command/Evidence/Hasil tanpa separator/table. Evidence wajib fenced raw snippet."
  run_step "14_beginner_app" "Beginner prompt test: user pemula minta bikin aplikasi sederhana di VPS. Jawab dalam format labeled blocks (Progress/Path/Command/Evidence/Hasil) dengan langkah ringkas runnable. Command WAJIB satu baris (tanpa heredoc panjang, tanpa dump full file body), gunakan path+verify evidence. Evidence WAJIB fenced raw snippet (bukan inline). Dilarang separator line (---) dan markdown table."
fi
